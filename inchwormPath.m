...
%     title('Inchworm Robot Walking Path with One-Block Step');
    ...
    %% walking path small step quintic
    pi_walk = [6, 0, 0];
    pf_walk = [9, 0, 0];
    vi_walk = [0, 0, 0.8];
    vf_walk = [0, 0, -0.8];
    ...
    ti_walk = 0;
    tf_walk = 6;
    t_walk = ti_walk: 0.05: tf_walk;
    
    x_coef = quinticTrajectory(ti_walk, tf_walk, pi_walk(1), pf_walk(1), 0,0,0,0)
    y_coef = quinticTrajectory(ti_walk, tf_walk, pi_walk(2), pf_walk(2), 0,0,0,0)
    z_coef = quinticTrajectory(ti_walk, tf_walk, pi_walk(3), pf_walk(3), vi_walk(3),vf_walk(3),0,0)
    th_coef = quinticTrajectory(ti_walk, tf_walk, pi, pi, 0,0,0,0);
    %path_walk = pathGen(pi_walk, pf_walk, vi_walk, vf_walk, ti_walk, tf_walk)
    x_path = x_coef(1) + x_coef(2) * t_walk + x_coef(3) * t_walk.^2 + x_coef(4) * t_walk .^3 + x_coef(5) * t_walk .^4 + x_coef(6) * t_walk.^5;
    y_path = y_coef(1) + y_coef(2) * t_walk + y_coef(3) * t_walk.^2 + y_coef(4) * t_walk .^3 + y_coef(5) * t_walk .^4 + y_coef(6) * t_walk.^5;
    z_path = z_coef(1) + z_coef(2) * t_walk + z_coef(3) * t_walk.^2 + z_coef(4) * t_walk .^3 + z_coef(5) * t_walk .^4 + z_coef(6) * t_walk.^5;
